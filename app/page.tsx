...
export default function TimeKeeperPage() {
  ...
  return (
    ...
    >
      ...
      {!isClockOnly && (
        <header
          className={`border-b ${themeMode === "dark" ? "border-zinc-800" : "border-zinc-700"} px-6 py-4`}
        >
          <div className="mx-auto max-w-7xl flex items-center justify-between">
            ...
            <div className="flex items-center gap-6">
              <button
                ...
                type="button"
                disabled={isLoading}
                className={`text-sm transition-colors flex items-center gap-2 ${themeMode === "dark" ? "text-muted-foreground hover:text-white" : "text-zinc-400 hover:text-zinc-100"} ${isLoading ? "opacity-50 cursor-not-allowed" : ""}`}
              >
                <RefreshCw
              ...
              <button
                ...
                className={`text-sm transition-colors ${themeMode === "dark" ? "text-muted-foreground hover:text-white" : "text-zinc-400 hover:text-zinc-100"}`}
              >
                THEME
              </button>
              <button
      ...
      <main className="mx-auto max-w-7xl px-6 py-12">
        ...
        {isLoading ? (
          <div className="text-center mb-16">
            <Card
              className={`${themeMode === "dark" ? "bg-zinc-900/50 border-zinc-800" : "bg-zinc-700/50 border-zinc-600"} p-12 backdrop-blur`}
            >
              <p
                className={`text-2xl ${themeMode === "dark" ? "text-muted-foreground" : "text-zinc-400"}`}
              >
          ...
          <div
            ref={clockRef}
            className={`text-center ${isClockOnly ? "min-h-screen flex flex-col items-center justify-center relative" : "mb-16"}`}
          >
            {isClockOnly ? (
              <Button
                ...
                size="lg"
                variant="outline"
                className={`absolute top-8 left-8 ${themeMode === "dark" ? "bg-zinc-900 border-zinc-700 hover:bg-zinc-800" : "bg-zinc-700 border-zinc-600 hover:bg-zinc-600"} gap-2 text-base px-8 h-12 font-medium`}
              >
              ...
              <div className="flex justify-center mb-6">
                <Button
                  ...
                  size="lg"
                  variant="outline"
                  className={`${themeMode === "dark" ? "bg-zinc-900 border-zinc-700 hover:bg-zinc-800" : "bg-zinc-700 border-zinc-600 hover:bg-zinc-600"} gap-2 text-base px-8 h-12 font-medium`}
                >
            ...
            <h2
              className={`${isClockOnly ? "text-7xl" : "text-5xl"} font-bold mb-2`}
            >
              {selectedExam.name}
            </h2>
            {selectedExam.source && (
              <p
                className={`text-sm mb-6 ${themeMode === "dark" ? "text-muted-foreground" : "text-zinc-400"}`}
              >
                Source: {selectedExam.source}
              </p>
            )}
            <Card
              className={`${themeMode === "dark" ? "bg-zinc-900/50 border-zinc-800" : "bg-zinc-700/50 border-zinc-600"} ${isClockOnly ? "p-20" : "p-12"} backdrop-blur`}
            >
              <div className="flex items-center justify-center gap-8">
                <div className="text-center">
                  <div
                    className={`${isClockOnly ? "text-9xl" : "text-8xl"} font-bold tabular-nums`}
                  >
                    {countdown.days}
                  </div>
                  <div
                    className={`text-sm mt-2 ${themeMode === "dark" ? "text-muted-foreground" : "text-zinc-400"}`}
                  >
                    days
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {filteredExams.map((exam) => {
              ...
              return (
                <Card
                  key={exam.id}
                  className={`${themeMode === "dark" ? "bg-zinc-900/50 border-zinc-800 hover:bg-zinc-900/70" : "bg-zinc-700/50 border-zinc-600 hover:bg-zinc-700/70"} p-6 cursor-pointer transition-colors`}
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-3xl font-bold tabular-nums">
                    <div className="text-center">
        ...
        {!isClockOnly && showFavorites && filteredExams.length === 0 && (
          <div
            className={`text-center mb-8 ${themeMode === "dark" ? "text-muted-foreground" : "text-zinc-400"}`}
          >
        ...
        {!isClockOnly && (
          <div className="flex justify-center">
            <Button
              variant="outline"
              size="lg"
              className={`w-full max-w-md ${themeMode === "dark" ? "bg-zinc-900 border-zinc-800 hover:bg-zinc-800" : "bg-zinc-700 border-zinc-600 hover:bg-zinc-600"} h-16 text-base`}
      ...
      {!isClockOnly && (
        <section
          className={`mx-auto max-w-7xl px-6 py-16 border-t ${themeMode === "dark" ? "border-zinc-800" : "border-zinc-700"}`}
        >
          ...
          <div className="grid gap-6 max-w-3xl mx-auto">
            <Card
              className={`${themeMode === "dark" ? "bg-zinc-900/50 border-zinc-800" : "bg-zinc-700/50 border-zinc-600"} p-6`}
            >
              <h3 className="text-lg font-semibold mb-3">
                Where does the exam data come from?
              </h3>
              <p
                className={`leading-relaxed ${themeMode === "dark" ? "text-muted-foreground" : "text-zinc-300"}`}
              >
      ...
      {!isClockOnly && (
        <footer
          className={`border-t ${themeMode === "dark" ? "border-zinc-800" : "border-zinc-700"} mt-20 py-8 px-6`}
        >
          <div
            className={`mx-auto max-w-7xl text-sm text-center ${themeMode === "dark" ? "text-muted-foreground" : "text-zinc-400"}`}
          >
